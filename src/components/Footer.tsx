...
export const Footer = () => {
  ...
  return (
    <footer className="bg-transparent text-slate-600 dark:text-slate-300 pt-20 pb-10 border-t border-slate-200 dark:border-slate-800 transition-colors duration-300 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2 select-none">
              {config.content[config.language].navbar.logoImage ? (
                <img src={config.content[config.language].navbar.logoImage} alt={`${config.content[config.language].navbar.logoText} - وسيط تسوق دولي نواكشوط`} className="h-10 object-contain dark:brightness-0 dark:invert" />
              ) : (
                <>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-4">
                ...
                <a href={content.youtubeUrl || '#'} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-red-600 hover:text-white dark:hover:bg-red-600 dark:hover:text-white transition-colors">
                  <Youtube size={20} />
                </a>
              ...
              {config.isAdminMode && (
                <div className="flex flex-col gap-2 mt-2 p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="flex items-center gap-2">
                    <Youtube size={14} className="text-slate-400 shrink-0" />
                    <EditableText section="footer" contentKey="youtubeUrl" value={content.youtubeUrl || ''} className="text-xs text-slate-600 dark:text-slate-400 w-full" />
                  </div>
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              <a href={config.content[config.language].appLinks?.ios} onClick={() => trackEvent('download', { platform: 'ios' })} target="_blank" rel="noopener noreferrer" className="block w-full transition-transform hover:scale-105 active:scale-95">
              ...
              <a href={config.content[config.language].appLinks?.android} onClick={() => trackEvent('download', { platform: 'android' })} target="_blank" rel="noopener noreferrer" className="block w-full transition-transform hover:scale-105 active:scale-95">
