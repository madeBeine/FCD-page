...
export const Navbar = () => {
  const { config, setLanguage, toggleAdminMode, isDarkMode, toggleDarkMode, saveConfig } = useConfig();
  ...
  const content = config.content[config.language].navbar;
  ...
  return (
    <header 
      className={cn(
        ...
        isScrolled ? "py-3 glass shadow-md dark:glass-dark" : "py-5 bg-transparent"
      )}
    >
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
        ...
        >
          {content.logoImage ? (
            <img src={content.logoImage} alt={content.logoText} className="h-10 object-contain dark:brightness-0 dark:invert" />
          ) : (
            <>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {content.links.map((link: any, index: number) => {
            ...
            const isActive = isPage ? location.pathname === `/${link.id}` : false;
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <button 
            ...
            className="bg-brand-blue hover:bg-brand-blue/90 text-white px-5 py-2.5 rounded-full font-medium text-sm transition-all hover:shadow-lg hover:shadow-brand-blue/30 flex items-center gap-2"
          >
            <Download size={18} />
            {content.downloadCta}
          </button>
          ...
          >
            <div className="container mx-auto px-4 py-4 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3 pt-2">
                ...
                <button 
                  onClick={() => {
                    handleDownload(config.content[config.language].appLinks);
                    setIsMobileMenuOpen(false);
                  }}
            ...
            >
              <div className="p-6">
                ...
                <p className="text-slate-500 dark:text-slate-400 mb-6">
                  {config.language === 'ar' ? 'هل أنت متأكد من رغبتك في الخروج؟ سيتم حفظ جميع التعديلات التي قمت بها.' : config.language === 'fr' ? 'Êtes-vous sûr de vouloir quitter ? Toutes vos modifications seront enregistrées.' : 'Are you sure you want to exit? All your changes will be saved.'}
                </p>
