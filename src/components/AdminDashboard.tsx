...
export const AdminDashboard = () => {
    ...
    try {
      ...
      if (supabase) {
        ...
        if (!visitsRes.error && visitsRes.data) {
          ...
          data = JSON.parse(localStorage.getItem('mock_analytics') || '[]');
        }

        if (!eventsRes.error && eventsRes.data) {
          eventsData = eventsRes.data;
        } else {
          eventsData = JSON.parse(localStorage.getItem('mock_analytics_events') || '[]');
        }
      } else {
        data = JSON.parse(localStorage.getItem('mock_analytics') || '[]');
        eventsData = JSON.parse(localStorage.getItem('mock_analytics_events') || '[]');
  ...
  const handleArrayChange = (section: string, key: string, index: number, subKey: string | null, value: any, currentArray: any[]) => {
    ...
    const sharedArrayKeys = ['logo', 'url', 'videoUrl', 'imageUrl', 'linkUrl', 'mediaType', 'videoThumbnail', 'icon', 'name', 'isVisible', 'colorClass', 'position', 'iconText'];
              ...
              <tbody className="text-sm text-slate-700">
                {analyticsData.slice(0, 10).map((visitor, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
                    ...
                    <td className="p-4">{visitor.city !== 'Unknown' ? `${visitor.city}, ${visitor.country}` : visitor.country}</td>
                    <td className="p-4">{visitor.device_type}</td>
                    <td className="p-4">{visitor.os}</td>
            ...
            <table className="w-full text-left border-collapse" dir="rtl">
              ...
              <tbody className="text-sm text-slate-700">
                {adminLogs.map((log, idx) => {
                  const loc = log.event_data?.exact_location || log.event_data?.location || {};
                  const lat = loc.lat ? loc.lat.toFixed(4) : 'N/A';
                  const lon = loc.lon ? loc.lon.toFixed(4) : 'N/A';
                  ...
                  return (
                    <tr key={idx} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
                      ...
                      <td className="p-4">{log.os}</td>
                      <td className="p-4">{loc.city && loc.city !== 'Unknown' ? `${loc.city}, ${loc.country}` : loc.country || 'Unknown'}</td>
                      <td className="p-4 font-mono text-xs" dir="ltr">{lat}, {lon}</td>
                    </tr>
                  );
                })}
              </tbody>
